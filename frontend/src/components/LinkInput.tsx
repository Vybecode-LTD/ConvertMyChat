...
const PAT = /^https?:\/\/(gemini\.google\.com\/share\/[a-zA-Z0-9]+|g\.co\/gemini\/share\/[a-zA-Z0-9]+)/;
...
export function LinkInput({ onSubmit, disabled }: { onSubmit: (url: string) => void; disabled?: boolean }) {
  ...
  const submit = () => {
    let u = url.trim();
    if (!u) { setErr("Paste a Gemini share link"); return; }
    if (!u.startsWith("http")) u = `https://${u}`;
    if (!PAT.test(u)) { setErr("Expected gemini.google.com/share/... or g.co/gemini/share/..."); return; }
    setErr("");
    onSubmit(u);
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <label htmlFor="share-link" className="block text-sm text-gray-400 mb-2">Gemini share link</label>
      <div className="flex gap-3">
        <input id="share-link" type="url" value={url}
          onChange={(e) => { setUrl(e.target.value); setErr(""); }}
          onKeyDown={(e) => e.key === "Enter" && !disabled && submit()}
          disabled={disabled} placeholder="Paste a gemini.google.com/share/... link"
